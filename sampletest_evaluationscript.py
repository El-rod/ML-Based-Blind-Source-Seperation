...
def run_demod_test(sig1_est, bit1_est, soi_type, interference_sig_type, testset_identifier):
    all_sig_mixture, all_sig1, all_bits1 = pickle.load(open(
        os.path.join('dataset', f'GroundTruth_{testset_identifier}_Dataset_{soi_type}_{interference_sig_type}.pkl'),
        'rb'))
...
if __name__ == "__main__":
    ...
    pickle.dump((mse_mean, ber_mean), open(os.path.join('outputs',
                                                        f'{id_string}_{testset_identifier}_exports_summary_{soi_type}_{interference_sig_type}.pkl'),
                                           'wb'))
