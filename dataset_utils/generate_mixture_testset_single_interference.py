"""
A python script for generating example mixtures for testing SOI+IPM for K=1;
focusing solely on the 11 discrete target SINR levels.
This script saves a pickle file dataset/Training_Dataset_[SOI Type]+[Interference Type].pkl'
...
def generate_demod_testmixture(soi_type, interference_sig_type, n_per_batch=default_n_per_batch, seed_number=0):
    ...
    pickle.dump((all_sig_mixture, all_sig1, all_bits1, meta_data),
                open(os.path.join('dataset', f'Dataset_Seed{seed_number}_{soi_type}+{interference_sig_type}.pkl'),
                     'wb'),
                protocol=4)
...
if __name__ == "__main__":
    ...
    generate_demod_testmixture('QPSK', 'CommSignal3', 2500, 1)
