...
def get_soi_generation_fn(soi_sig_type):
    if soi_sig_type == 'QPSK':
        ...
        with h5py.File(os.path.join('dataset', 'interferenceset_frame', soi_sig_type + '_raw_data.h5'),
                       'r') as data_h5file:
...
def generate_dataset(sig_data, soi_type, interference_sig_type, sig_len, n_examples, n_per_batch, foldername, seed,
                     verbosity):
    random.seed(seed)
    np.random.seed(seed)
